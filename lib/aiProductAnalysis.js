...
export async function analyzeProductWithAI(productInfo) {
  ...
  try {
    ...
    return {
      ...
      harmfulSubstances: analysis.harmfulSubstances || [],
      microplasticsRisk: analysis.microplasticsRisk || 'unknown',
      microplasticsReason: analysis.microplasticsReason || '',
      heavyMetalsRisk: analysis.heavyMetalsRisk || 'unknown',
      heavyMetalsReason: analysis.heavyMetalsReason || '',
      allergenInfo: analysis.allergenInfo || [],
      ageAppropriateness: analysis.ageAppropriateness || 'Unknown',
      nutritionalConcerns: analysis.nutritionalConcerns || [],
      recommendations: analysis.recommendations || '',
      confidenceScore: analysis.confidenceScore || 75,
      beneficialIngredients: analysis.beneficialIngredients || { count: 0, details: [] },
...
function buildAnalysisPrompt(productInfo) {
  const hasIngredients = productInfo.ingredients && productInfo.ingredients !== 'Not provided';

  // If no ingredients, provide context about analyzing by product type
  const analysisContext = !hasIngredients ? `
IMPORTANT: No ingredients list was provided. Please analyze based on:
1. Typical ingredients for "${productInfo.name}" products from ${productInfo.brand}
2. Common formulations for this type of baby food
3. Known product information for ${productInfo.brand} products
4. General safety profile of similar products

Note: This is a general analysis based on product type. Actual ingredients may vary.
` : '';
...
${productInfo.ingredients ? `- Ingredients: ${productInfo.ingredients}` : '- Ingredients: Not provided (analyze based on typical formulation)'}
...
${productInfo.analysisNote ? `\nNote: ${productInfo.analysisNote}` : ''}
${analysisContext}
...

{
  ...
  "heavyMetalsRisk": "<none|low|medium|high>",
  "heavyMetalsReason": "Brief explanation of heavy metal risk",
  "allergenInfo": ["List of allergens present"],
  "ageAppropriateness": "Age range this product is suitable for (e.g., '6+ months', '8-10 months')",
  "nutritionalConcerns": ["List of nutritional concerns"],
  ...
  "confidenceScore": <number 50-100, lower if no ingredients available>
}
...
function validateAnalysisStructure(analysis) {
  ...
  // Validate arrays
  if (!Array.isArray(analysis.harmfulSubstances)) {
    throw new Error('harmfulSubstances must be an array');
  }
  ...
  if (analysis.beneficialIngredients &&
      (analysis.beneficialIngredients.count === undefined ||
  ...
  if (analysis.microplasticsRisk && !validRisks.includes(analysis.microplasticsRisk)) {
    throw new Error(`microplasticsRisk must be one of: ${validRisks.join(', ')}`);
  }

  if (analysis.heavyMetalsRisk && !validRisks.includes(analysis.heavyMetalsRisk)) {
    throw new Error(`heavyMetalsRisk must be one of: ${validRisks.join(', ')}`);
  }
}
